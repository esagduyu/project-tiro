...
**Completed:** 21 checkpoints + UX redesign Phase 1 (foundation) in progress
...
- **Keyboard navigation** (Checkpoint 11+): Full keyboard-first navigation. Inbox: `j`/`k` move selection, `Enter` opens article, `s` toggles VIP, `1`/`2`/`3` rate (dislike/like/love), `/` focuses search, `d` switches to digest, `a` switches to articles, `c` classify/reclassify, `g` go to stats, `v` go to graph, `?` shows shortcuts overlay. Digest view: `r` generates or regenerates digest. Reader: `b`/`Esc` goes back, `s` toggles VIP, `1`/`2`/`3` rate, `p` play/pause audio, `i` toggles analysis panel, `r` runs/re-runs analysis (when panel open), `g` go to stats, `v` go to graph, `?` shows shortcuts. Stats page: `b`/`Esc` back to inbox, `e` export, `v` go to graph, `?` shortcuts. Settings page: `b`/`Esc` back, `v` go to graph, `?` shortcuts. Graph page: `b`/`Esc` back, `?` shortcuts. Keys are ignored when focus is on input/select elements. Selected article gets `.kb-selected` highlight class. Shortcuts overlay in `base.html` (shared), populated by JS per view.
...
- **UX Redesign (Checkpoint 22, in progress):** Major overhaul across 8 phases (16 tasks). Phase 1 complete: schema migration (`ingestion_method` column with backfill via `migrate_db()`), config fields (`inbox_page_size`, `theme_light`, `theme_dark`), theme CSS files (`tiro/frontend/static/themes/papyrus.css` + `roman-night.css` with 20 `--tiro-*` variables each), base.html rewritten with left sidebar nav (220px, icons+labels, unread badge, dark mode toggle, About Tiro link, mobile header with hamburger), styles.css fully rewritten to use `--tiro-*` variables (zero hardcoded colors). Roman color palette: papyrus cream `#FAF6F0` bg, terra cotta `#C45B3E` accent, olive `#6B7F4E` secondary, warm gold `#B8943E` for links/VIP. Dark mode: Roman Night with warm charcoal `#1C1A17` bg. Remaining phases: routing (extract Digest to /digest, /inbox route), API (filters + pagination), frontend (filter panel, Roman numeral pagination, updated cards), settings (appearance/themes), MCP (enhanced search + list_filters), responsive, docs + testing. Full plan in `docs/plans/2026-02-16-ux-redesign-plan.md`.
- **Browser cache busting**: Currently at v=30 in base.html and reader.html. ALWAYS increment when modifying static files.
