...
- **IMAP sync scheduler** (Checkpoint 20): `asyncio` background task in FastAPI lifespan polls IMAP inbox every N minutes via `check_imap_inbox()`. Config: `imap_sync_interval: int = 0` (minutes, 0 = manual only). Default set to 15 when IMAP is configured via `tiro init` or `tiro setup-email`. The background loop reads `config.imap_sync_interval` each cycle so Settings page changes take effect on the next cycle without server restart. Setting interval to 0 stops the loop. Logs results at INFO (if messages found) or DEBUG (no messages). Settings page email section shows "Auto-sync every N min" or "Manual only" in the IMAP status card. Configure email modal has an "Auto-sync interval" dropdown (Manual / 5 / 10 / 15 / 30 / 60 min). Label and interval fields hide/show when Receive checkbox is toggled.
